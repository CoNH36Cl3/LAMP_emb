...
def write_chk(mc,root,chkfile):
    ...
    if mc._scf._eri is None:
        ...
        nmo,ncas,mo=mc.mo_coeff.shape[0],mc.ncas,mc.mo_coeff
        h2e_t = numpy.zeros((nmo,ncas,ncas,ncas),dtype=numpy.float64)
        ...
        for eri1 in mc._scf.with_df.loop():
            ...
            h2e_t += lib.einsum('Pij,Pkl->ijkl', Lij[:,:,ncore:nocc], Lij[:,ncore:nocc,ncore:nocc])
        #
